...
- **Cloud or local,** open a **new** session on this repo (Opus 5.5, medium effort, clean context) and say **"continue"**. Each session does one milestone part within 400–600k tokens. The session loads `docs/memory/NOW.md` automatically. A startup hook finds the newest work, even on an unmerged branch or in a stale local clone, and the `start-session` skill carries on from there.
