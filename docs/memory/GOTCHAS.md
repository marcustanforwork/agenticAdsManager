...
- **Commander's `exitOverride()` and `configureOutput()` are not inherited by subcommands that already exist** (`addCommand`). Tests apply them recursively (`quiet()` in `apps/*/test/credentials.test.ts`). *(M01b, 2026-09-25)*
- **zod 4's `z.record` drops a `__proto__` key** when parsing, so a `settings_patch` can't carry one; the merge refuses it anyway. `ProductSettings` (a plain `z.object`) **strips unknown keys silently**, so the processor compares input and output to refuse typos by name. *(M01b, 2026-09-25)*
- **Postgres behaviours the queue relies on are proven by tests, not docs** (postgresql.org is blocked in cloud sessions): NOTIFY inside a transaction arrives at commit (`queue.test.ts`); a session advisory lock is freed when its connection is killed (`runner-leader.test.ts`). *(M01b, 2026-09-25)*
- **Node AES-256-GCM:** set `authTagLength: 16` explicitly, call `setAAD` before `update`, and `getAuthTag` after `final`; on decrypt, `setAuthTag` before `final`, which throws if the tag doesn't match. The vault uses a 12-byte random IV (the GCM standard). *(nodejs.org v24 crypto docs, 2026-09-25)*
