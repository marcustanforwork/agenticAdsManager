...
| T3 | **Doppler:** a project with `dev`, `worker`, `gateway` and `dashboard` configs; generate the two vault master keys (read, write). From M07: a **service token** each for `worker` and `gateway`, in a `.env` file next to `docker-compose.yml` on the SER9 (D-065) | M01a live steps | Never give Claude sessions the `worker`/`gateway` configs |
