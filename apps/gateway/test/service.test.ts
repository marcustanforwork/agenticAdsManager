...
describe('gateway service', () => {
  it('serves /healthz on localhost and stops', async () => {
    ...
    try {
      ...
      expect(await res.json()).toEqual({ status: 'ok', service: 'gateway' });
...
describe('ads-gw CLI', () => {
  it('prints the version', async () => {
    ...
    await buildProgram().exitOverride().parseAsync(['node', 'ads-gw', '--product', 'demo-product', 'version']);
    expect(log).toHaveBeenCalledWith(expect.stringMatching(/^ads-gw \d+\.\d+\.\d+$/));
    log.mockRestore();
  });
  ...
  it('rejects a malformed --product', async () => {
    ...
    await expect(program.parseAsync(['node', 'ads-gw', '--product', 'Bad Slug!', 'version'])).rejects.toThrow();
  });
});
