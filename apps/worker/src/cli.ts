...
const defaultDeps: CliDeps = {
  ...
  connect: (url) => connect(url, { max: 1, applicationName: 'ads-cli' }),
};

/** This CLI's key: the read master key, which opens read credentials only. */
const KEY_ENV = 'VAULT_READ_KEY';
const ROLES: CredentialRole[] = ['read'];
...
function credentialsCommand(deps: CliDeps): Command {
  ...
  credentials
    ...
    .action(async (opts: { account: string; role: CredentialRole }) => {
      const masterKey = masterKeyFromEnv(KEY_ENV, 'read', deps.env);
  ...
  credentials
    ...
    .action(async () => {
      const from = masterKeyFromEnv(KEY_ENV, 'read', deps.env);
      const to = masterKeyFromEnv(`${KEY_ENV}_NEW`, 'read', deps.env);
