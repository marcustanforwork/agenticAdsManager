...
- **Before M00** (the repo is docs only): skip to step 2 and check that every file path you reference exists.
- **From M00:**
  ```bash
  pnpm typecheck && pnpm lint && pnpm check:boundaries && pnpm test
  ```
  M00 replaces this block with the final command list.

Everything must be green, or the failure must be reported. **Never claim green without having seen the output.**

## 2. Fast grep checks (from M00)
...
grep -l 'connector-google-write\|connector-meta-write' packages/*/package.json packages/packs/*/package.json apps/*/package.json 2>/dev/null | grep -v '^packages/gateway/'
# Invariant 2: no product names in shared code
grep -rniE 'snappool|property-sg|sora' packages/core packages/gateway packages/connector-* packages/vault packages/db apps/web --include='*.ts' --include='*.tsx' 2>/dev/null
# Invariant 6: floats near money (review each hit)
grep -rnE 'parseFloat|Math\.round|Number\(' packages apps --include='*.ts' 2>/dev/null | grep -iE 'micros|spend|budget|amount|value'
...
M00 adjusts the package names and paths if the real ones differ.
